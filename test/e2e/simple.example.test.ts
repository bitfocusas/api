...
describe('Simple Example E2E', () => {
	...
	it('should run the simple example successfully', async () => {
		...
		const response1 = await supertest(app.instance.server).get('/hello').expect(200)

		expect(response1.body).toEqual({ message: 'Hello, World!' })

		// Test with name parameter
		const response2 = await supertest(app.instance.server).get('/hello?name=Alice').expect(200)
	...
	it('should handle query parameter validation', async () => {
		...
		const response2 = await supertest(app.instance.server).get('/hello?name=John%20Doe').expect(200)
